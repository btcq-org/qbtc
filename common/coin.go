...
func NewCoin(asset Asset, amount math.Uint) Coin {
	return Coin{
		Asset:  asset,
		Amount: amount,
	}
...
func ParseCoin(coinStr string) (Coin, error) {
	...
	// parse the amount
	amount, err := math.ParseUint(split[0])
	if err != nil {
...
// IsNative check whether the coin is native on BTCQ chain
...
func (c Coin) Native() (sdk.Coin, error) {
	if !c.IsNative() {
		return sdk.Coin{}, errors.New("coin is not on BTCQ chain")
	}
	return sdk.NewCoin(
		c.Asset.Native(),
		math.NewIntFromBigInt(c.Amount.BigInt()),
	), nil
}
...
func (cs Coins) SafeSub(subCoins ...Coin) Coins {
	// Make a new slice to not affect 'oldCoins', inheriting the item order of 'oldcoins'.
	newCoins := NewCoins(cs...)
