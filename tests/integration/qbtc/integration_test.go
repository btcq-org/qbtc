...
// advanceBlockWithTime advances the chain by one block with a time jump.
func (s *IntegrationTestSuite) advanceBlockWithTime(timeJump time.Duration) {
	header := s.ctx.BlockHeader()
	header.Height++
	header.Time = header.Time.Add(timeJump)
...
// submitTx signs and submits a transaction, returning the result.
func (s *IntegrationTestSuite) submitTx(msg sdk.Msg, signer sdk.AccAddress, signerPrivKey cryptotypes.PrivKey) (*abci.ExecTxResult, error) {
	return s.ExecuteMsg(msg, signer, signerPrivKey)
}

// submitTxExpectSuccess submits a transaction and expects it to succeed.
func (s *IntegrationTestSuite) submitTxExpectSuccess(msg sdk.Msg, signer sdk.AccAddress, signerPrivKey cryptotypes.PrivKey) *abci.ExecTxResult {
	result, err := s.submitTx(msg, signer, signerPrivKey)
	s.Require().NoError(err)
	s.Require().NotNil(result)
	s.Require().True(result.IsOK(), "tx failed: %s", result.Log)
	return result
}

// submitTxExpectFail submits a transaction and expects it to fail.
func (s *IntegrationTestSuite) submitTxExpectFail(msg sdk.Msg, signer sdk.AccAddress, signerPrivKey cryptotypes.PrivKey) *abci.ExecTxResult {
	result, err := s.submitTx(msg, signer, signerPrivKey)
	// Either the submission itself fails, or the tx fails during execution
	if err != nil {
		return nil
	}
	s.Require().NotNil(result)
	s.Require().False(result.IsOK(), "tx should have failed but succeeded")
	return result
}
